...
### 1. Через `run_main.js` (удобно для ручных настроек)

В `run_main.js` правите переменные (`groupId`, `startCount` и т.д.), затем:
...
`run_main.js` сам вызовет `main.js` и передаст все параметры аргументами.
...
```
Session [YYYY.MM.DD HH⁚mm⁚ss.SSS] Название группы/
├── Контент/
├── Ссылки на видео из группы Название группы.txt
├── Ссылки на GIF из группы Название группы.txt
├── Информация о сборе группы Название группы.txt
└── Точка продолжения группы Название группы.txt
```

В `Контент/` сохраняются фото, полные тексты и опросы. Имена содержат VK id поста,
поэтому одинаковые дата и текст не перезаписывают файлы. В корне Session лежат
упорядоченные списки ссылок, точный offset для продолжения и отчёт о сборе
(параметры старта, итоги батчей, финальная сводка из лога). Если стена пройдена
полностью, вместо точки продолжения создаётся файл об успешном завершении стены.

## Устойчивость запросов

- Один `wall.get` получает сразу `startCount` постов; отдельный запрос на каждый пост не выполняется.
- Перед сохранением батч последовательно проверяется по дате, `allCount` и опросу, затем выбранные посты обрабатываются пулом до 6 параллельных задач.
- Результаты GIF/video объединяются в исходном порядке стены независимо от порядка завершения загрузок.
- Следующий батч запрашивается только после полного сохранения текущего.
- Временные сетевые ошибки и лимиты VK повторяются с увеличивающейся паузой.
- У запросов есть timeout; изображения проверяются по HTTP-статусу, MIME и размеру.
- Завершение стены определяется по содержимому ответа и `response.count`, а не по скорости ответа.
...
| Не указан groupId | Передайте `--groupId=...` или задайте группу в `run_main.js` |
| Не удалось получить информацию о группе | Проверьте правильность `groupId` / ссылки и токен |
| `startCount` вне диапазона | Укажите целое число от 1 до 100 |
| Часть фото не сохранилась | Проверьте предупреждения: парсер продолжает остальные файлы и считает ошибки в сводке |

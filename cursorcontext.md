...
- **`main.js`** — оркестратор парсинга. Получает стену пакетами, последовательно обрабатывает посты и полностью ожидает запись текущего батча перед следующим `wall.get`.
- **`main_utils.js`** — чистая логика CLI, дат, безопасных имён, нормализации `copy_history`, классификации вложений и условий остановки.
...
## Важные особенности
- Один `wall.get` получает `startCount` постов. Посты ответа сохраняются последовательно для точной остановки по дате/allCount/опросу; фото одного поста загружаются параллельно.
...
## Тесты
- `tests/main_utils.test.js` — CLI, даты, вложенные repost, имена и условия остановки.
